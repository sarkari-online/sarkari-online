...
class CategoryService {
    ...
    public static function autoResolveCategory(string $title, string $content = '', ?string $currentSlug = null): ?array {
        ...
        // 1. College Updates & Higher Education (UGC, PhD Admission, Academic Calendar, IIT, NIT, University, JoSAA, DU CSAS, Fee Refund, Gap Certificate)
        if (str_contains($t, 'phd admission') || str_contains($t, 'academic calendar') || str_contains($t, 'ugc') || str_contains($t, 'fee refund') || str_contains($t, 'gap certificate') || str_contains($t, 'josaa') || str_contains($t, 'csab') || str_contains($t, 'csas') || str_contains($t, 'seat allotment') || str_contains($t, 'iit ') || str_contains($t, 'nit ') || str_contains($t, 'university') || str_contains($t, 'campus') || str_contains($t, 'higher education') || str_contains($t, 'college update') || str_contains($t, 'college admission')) {
            $slug = 'college-updates';
        }
        // 2. Scholarships & Fellowships (NSP, PMSSS, PM Yasasvi, Post-Matric, Stipends)
        elseif (str_contains($t, 'scholarship') || str_contains($t, 'fellowship') || str_contains($t, 'nfsc') || str_contains($t, 'nsp') || str_contains($t, 'pmsss') || str_contains($t, 'yasasvi') || str_contains($t, 'stipend') || str_contains($t, 'financial aid') || str_contains($t, 'post matric') || str_contains($t, 'pre matric') || str_contains($t, 'fee waiver')) {
            $slug = 'scholarships';
        }
        // 3. Student Tech & AI (DigiLocker, ABC, APAAR, Aadhaar, NSDC, OTR)
        ...
        // 4. Career Guides & Preparation Strategy (Syllabus, Weightage, Roadmap, Strategy)
